...
- Comments are Photon-only for new entries via `photon_capsule`, `photon_bytes`, `photon_mode`, and `photon_encoding` columns. Photon comment playback stores generated audio permanently in `audio_data` under a Photon cache key after generation/first play, so future playback is instant while still keeping the Photon capsule metadata.
