...
- Comments are Photon-only for new entries via `photon_capsule`, `photon_bytes`, `photon_mode`, and `photon_encoding` columns. Photon comment playback avoids writing generated audio into SQLite: it regenerates from text + capsule and uses a bounded in-memory replay cache for fast repeat playback during the current server session. Legacy non-Photon comments may still use `audio_data`.
